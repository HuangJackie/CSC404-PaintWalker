...
public class MoveRedo : ScriptableObject, IRedoCommand
{
    ...
    public void PlayerInit(GameObject player, CameraRotation cameraToMove,
                           Vector3 player_destination, Quaternion player_rotation)
    {
        this.player = player;
    ...
    public void ObjectInit(GameObject obj)
    {
        ...
        foreach (GameObject i in this.objects)
        {
            ...
            if (i.gameObject.CompareTag("Ground"))
            {
                ...
                this.blockMetadata.Add((groundScript.isPaintedByFeet, groundScript.isPaintedByBrush,
                                       ((Interactable) groundScript).originalColour, groundScript._paintedColour));
            }
        }
    }
    ...
    public void Undo()
    {
            ...
            foreach (GameObject obj in objects)
            {
                if (obj != null)
                {
                    ...
                    if (obj.gameObject.CompareTag("Ground"))
                    {
                        ...
                        {
                            obj.gameObject.GetComponentInChildren<Renderer>().material.color =
                                new Color(1.000f, 0.993f, 0.816f);
                        }
                        else
                        {
                            obj.gameObject.GetComponentInChildren<Renderer>().material.color =
                                blockMetadata[ground_index].Item3;
                        }
                        ground_index++;
                    }
        ...
        leveManager.IncreasePaint(this.paintTypeSpent, this.amountSpent);
    }

    public void RecordPaintSpent(Paints paintType, int amount)
    {
        this.amountSpent = amount;
        this.paintTypeSpent = paintType;
    }
